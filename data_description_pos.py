...
def part_of_speech(pos):
    ...
    for item, key in pos_tag:
        if "VB" in key or "NN" in key or "JJ" in key:
            key = key[0:2]
...
def count_values(pos_dict):
    ...
    for pos, freq in pos_dict.items():
        if pos=="VB" or pos=="NN" or pos=="JJ":
            print '------' + pos + '-------'
            print sort_dict(freq)[0:20]
...
def main():
    ...
    print "POSITIEF"
    pos_in = open("positive.pickle","rb")
    pos_list = pickle.load(pos_in)
    lijst1 = part_of_speech(pos_list)
    count_values(lijst1)

    print "NEGATIEF"
    neg_in = open("negative.pickle","rb")
